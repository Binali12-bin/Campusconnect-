...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    if (existingAdmin) {
      return res.status(403).json({
        success: false,
        message: "This email is reserved for an administrator"
      });
    }
    ...
    // Trigger normally creates this row automatically.
    // We also make sure the profile exists.
    ...
    res.status(201).json({
      ...
      user: {
        ...
        phone: student.phone,
        role: "student",
        wallet_balance_kobo: student.wallet_balance_kobo
      }
    });
  } catch (error) {
    console.error(error);
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    if (admin) {
      ...
      if (authData.user.id !== admin.auth_user_id) {
        return res.status(403).json({
          success: false,
          message: "Administrator account is not correctly linked"
        });
      }
...
// PUBLIC AUTHENTICATED NOTES LIST
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        return res.status(400).json({
          success: false,
          message: "Course, title, price and file are required"
        });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (price_kobo !== undefined) {
        updateData.price_kobo = Number(price_kobo);
      }
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const amount = Number(note.price_kobo);

      if (Number(student.wallet_balance_kobo) < amount) {
        return res.status(400).json({
          success: false,
      ...
      // This endpoint is intentionally kept simple until
      // the secure database transaction function is installed.
      const newBalance =
        Number(student.wallet_balance_kobo) - amount;
      ...
      const { error: balanceError } = await supabase
        ...
        .eq("id", student.id)
        .eq("wallet_balance_kobo", student.wallet_balance_kobo);
      ...
      if (purchaseError) {
        // Attempt to restore balance if purchase insertion fails.
        await supabase
          .from("students")
          .update({
            wallet_balance_kobo: student.wallet_balance_kobo
          ...
          .eq("id", student.id)
          .eq("wallet_balance_kobo", newBalance);
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        wallet_balance_kobo: data.wallet_balance_kobo
      });
    } catch (error) {
      res.status(500).json({
        success: false,
  ...
  async (req, res) => {
    try {
      const [
        ...
        supabase.from("purchases").select("id, amount_kobo"),
        supabase.from("payments").select("id, amount_kobo, status")
      ]);

      const totalSales =
        (purchasesResult.data || []).reduce(
          (sum, item) => sum + Number(item.amount_kobo || 0),
          0
        );
      ...
      res.json({
        ...
        stats: {
          ...
          total_wallet_funding_kobo: totalWalletFunding
        }
      });
    } catch (error) {
      console.error(error);
